...
bool AttrSetHandler_callback(const uint8_t requestId, const uint16_t attributeId, const uint16_t valueLen, const uint8_t *value)
{
    fprintf(stdout, "AttrSetHandler_callback: %d len: %d %x %s\n", attributeId, valueLen, *value, value);
    switch (attributeId)
    {
    case AF_BLINK:
        blinking = (*value == 1);
...
void AttrNotifyHandler_callback(const uint8_t requestId, const uint16_t attributeId, const uint16_t valueLen, const uint8_t *value)
{
    fprintf(stdout, "AttrNotifyHandler_callback: %d len: %d %s\n", attributeId, valueLen, value);
    fprintf(stdout, "AttrNotifyHandler_callback\n\n");
...
int main(int argc, char *argv[])
{
    ...
    theLib = iafLib::create(0, isr_callback, AttrSetHandler_callback, AttrNotifyHandler_callback, theLog, theSPI);
    theLib->mcuISR();
