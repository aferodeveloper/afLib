...
afLib::afLib(int chipSelect, int mcuInterrupt, isr isr_wrapper,
  ...
  attachInterrupt(mcuInterrupt, isr_wrapper, FALLING);
}

void afLib::loop(void) {
  if (isIdle() && (queueGet(&_request.messageType, &_request.requestId, &_request.attrId, &_request.valueLen, &_request.p_value) == afSUCCESS)) {
...
int afLib::getAttribute(uint16_t attrId) {
  _requestId++;
 ...
 */
int afLib::setAttribute(uint16_t attrId, uint8_t value) {
  _requestId++;
  return queuePut(IS_MCU_ATTR(attrId) ? MSG_TYPE_UPDATE : MSG_TYPE_SET, _requestId, attrId, sizeof(value), &value);
}

int afLib::setAttribute(uint16_t attrId, uint16_t value) {
  _requestId++;
  return queuePut(IS_MCU_ATTR(attrId) ? MSG_TYPE_UPDATE : MSG_TYPE_SET, _requestId, attrId, sizeof(value), (uint8_t *)&value);
}

int afLib::setAttribute(uint16_t attrId, uint32_t value) {
  _requestId++;
  return queuePut(IS_MCU_ATTR(attrId) ? MSG_TYPE_UPDATE : MSG_TYPE_SET, _requestId, attrId, sizeof(value), (uint8_t *)&value);
}

int afLib::setAttribute(uint16_t attrId, uint64_t value) {
  _requestId++;
  return queuePut(IS_MCU_ATTR(attrId) ? MSG_TYPE_UPDATE : MSG_TYPE_SET, _requestId, attrId, sizeof(value), (uint8_t *)&value);
}

int afLib::setAttribute(uint16_t attrId, String value) {
  _requestId++;
  return queuePut(IS_MCU_ATTR(attrId) ? MSG_TYPE_UPDATE : MSG_TYPE_SET, _requestId, attrId, value.length(), (uint8_t *)value.c_str());
}

int afLib::setAttribute(uint16_t attrId, uint16_t valueLen, const char *value) {
  _requestId++;
  return queuePut(IS_MCU_ATTR(attrId) ? MSG_TYPE_UPDATE : MSG_TYPE_SET, _requestId, attrId, valueLen, (const uint8_t *)value);
}

int afLib::setAttribute(uint16_t attrId, uint16_t valueLen, const uint8_t *value) {
  _requestId++;
  return queuePut(IS_MCU_ATTR(attrId) ? MSG_TYPE_UPDATE : MSG_TYPE_SET, _requestId, attrId, valueLen, value);
}

int afLib::setAttributeComplete(uint8_t requestId, uint16_t attrId, uint16_t valueLen, const uint8_t *value) {
...
int afLib::parseCommand(const char *cmd) {
    ...
    if (!_writeCmd->isValid()) {
      Serial.print("BAD: "); Serial.println(cmd);
      _writeCmd->dumpBytes();
      _writeCmd->dump();
      delete(_writeCmd);
      _writeCmd = NULL;
      ...
      case STATE_CMD_COMPLETE:
        ...
        if (_readCmd != NULL) {
          ...
          switch (_readCmd->getCommand()) {
          	...
          	case MSG_TYPE_UPDATE:
              _onAttrSetComplete(_readCmd->getReqId(), _readCmd->getAttrId(), &val);
          	  break;
...
//    if (_state != STATE_IDLE && millis() - timeout > 10000) {
//      _state = STATE_IDLE;
//      mcuISR();
//    }
  }
...
int afLib::exchangeStatus(StatusCommand *tx, StatusCommand *rx) {
  ...
  if (cmd != 0x30 && cmd != 0x31) {
    Serial.print("exchangeStatus bad cmd: "); Serial.println(cmd, HEX);
...
int afLib::writeStatus(StatusCommand *c) {
  ...
  byte cmd = SPI.transfer(bytes[index++]);
  if (cmd != 0x30 && cmd != 0x31) {
    Serial.print("writeStatus bad cmd: "); Serial.println(cmd, HEX);
    result = afERROR_INVALID_COMMAND;
  }
...
void afLib::dumpBytes(char *label, int len, uint8_t *bytes) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (b < 0x10) {
      Serial.print("0x0"); Serial.print(b, HEX);
    } else {
...
int afLib::queuePut(uint8_t messageType, uint8_t requestId, uint16_t attributeId, uint16_t valueLen, const uint8_t *value) {
  for (int i = 0; i < REQUEST_QUEUE_SIZE; i++) {
...
int afLib::queueGet(uint8_t *messageType, uint8_t *requestId, uint16_t *attributeId, uint16_t *valueLen, uint8_t **value) {
  for (int i = 0; i < REQUEST_QUEUE_SIZE; i++) {
    if (_requestQueue[i].p_value != NULL) {
