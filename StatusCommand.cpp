...
bool StatusCommand::equals(StatusCommand *statusCommand) {
  return (_cmd == statusCommand->_cmd && _bytesToSend == statusCommand->_bytesToSend && _bytesToRecv == statusCommand->_bytesToRecv);
}
...
void StatusCommand::dump() {
  Serial.print("cmd              : "); Serial.println(_cmd == 0x30 ? "STATUS" : "STATUS_ACK");
  Serial.print("bytes to send    : "); Serial.println(_bytesToSend, DEC);
  Serial.print("bytes to receive : "); Serial.println(_bytesToRecv, DEC);
}
